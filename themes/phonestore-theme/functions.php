...
// Override single product content
function phonestore_override_single_product_summary() {
...
add_action('init', 'phonestore_override_single_product_summary');

// Ensure WooCommerce templates are loaded
function phonestore_woocommerce_locate_template($template, $template_name, $template_path) {
...
add_filter('woocommerce_locate_template', 'phonestore_woocommerce_locate_template', 10, 3);

// Add product specifications meta fields (for demo purposes)
function phonestore_add_demo_product_specs($product_id) {
    // Only add if not already set
    if (!get_post_meta($product_id, 'brand', true)) {
        $demo_specs = array(
            'brand' => 'Apple',
            'display_size' => '6.1 inch',
            'cpu' => 'Apple A17 Pro',
            ...
            'colors' => 'Xanh Titan, Trắng Titan, Đen Titan, Vàng Titan'
        );
        
        foreach ($demo_specs as $key => $value) {
            update_post_meta($product_id, $key, $value);
        }
    }
}

// Auto-add demo specs to products (for testing)
function phonestore_auto_add_specs_to_products() {
    if (isset($_GET['add_demo_specs'])) {
        ...
        foreach ($products as $product) {
            phonestore_add_demo_product_specs($product->ID);
        }
        
        echo '<div style="background: green; color: white; padding: 10px; margin: 20px; text-align: center;">Demo specs đã được thêm vào tất cả sản phẩm!</div>';
    }
}
add_action('wp_head', 'phonestore_auto_add_specs_to_products');
?>